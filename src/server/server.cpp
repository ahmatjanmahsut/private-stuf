...
void Server::run() {
    ...
    asio::ip::tcp::endpoint ep(
        asio::ip::make_address(cfg_.listen_host), cfg_.listen_port);
...
void Server::handle_handshake(std::shared_ptr<asio::ip::tcp::socket> sock) {
    ...
    // 创建 crypto，完成 ECDH（server 直接做完整 handshake）
    ...
    uint32_t sid = tunnel_mgr_.create_session(std::move(crypto),
                                               std::move(obfuscators));
...
// ── 会话数据收发（client → server → TUN）────────────────────────────────
void Server::run_session(std::shared_ptr<asio::ip::tcp::socket> sock,
                          uint32_t session_id) {
...
void Server::tun_read_loop() {
    ...
    while (running_) {
        ...
        for (auto& [sid, sock] : snapshot) {
            ...
            asio::error_code ec;
            asio::write(*sock, asio::buffer(len_hdr, 4), ec);
            if (!ec) asio::write(*sock, asio::buffer(encrypted), ec);
            if (ec)
                VPN_WARN("tun_read_loop: send to session {} failed: {}",
                         sid, ec.message());
        }
    }
}
